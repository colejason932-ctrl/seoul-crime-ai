...
if st.session_state.selected_menu == "deepdive":
    ...
    with col1:
        ...
        st.warning("**[AI 모델 분석 원리]**\n본 모델은 유동인구와 인프라 간의 상호작용을 학습한 XGBoost를 사용합니다. 자치구별 취약 요인이 다르게 나타납니다.")
        st.markdown(f"""
    ...
    with col2:
        # [변경됨] 선택한 자치구의 실제 데이터 값을 정규화하여 그래프로 표시
        radar_data = pd.DataFrame({
            '지표': features,
            '현재값_상대비율': [(gu_data[f] / df[f].max() * 100) if df[f].max() > 0 else 0 for f in features]
        }).sort_values('현재값_상대비율', ascending=True)

        fig_local_imp = px.bar(
            radar_data, 
            x='현재값_상대비율', 
            y='지표', 
            orientation='h', 
        ...
        fig_local_imp.update_traces(marker_color='#e84393') # 빨간색 계열로 변경
...
# ==========================================
# 5. 메뉴 2: CCTV 우선순위 (자치구 강조 시각화)
# ==========================================
elif st.session_state.selected_menu == "budget":
    st.subheader(f"🎯 {selected_gu} 치안 예산 최적화: CCTV 설치 우선순위 리포트")
    st.markdown("예측 위험도와 인프라 보강 필요도를 기반으로 예산 투입이 가장 시급한 지역을 선별합니다.")
    
    priority_df = df.sort_values('CCTV_설치_우선순위_점수', ascending=False)
    
    # 선택된 자치구의 순위 찾기
    gu_rank = priority_df.reset_index(drop=True)[priority_df.reset_index(drop=True)['자치구'] == selected_gu].index[0] + 1
    total_gu = len(priority_df)
    
    c1, c2, c3 = st.columns(3)
    with c1: st.error(f"### 🚨 긴급 설치 1순위\n{priority_df.iloc[0]['자치구']}")
    with c2: st.info(f"### 📍 현재 선택 지역\n**{selected_gu}** (전체 {total_gu}개 중 **{gu_rank}위**)")
    with c3: st.success(f"### ✅ 유지/관리 구역\n{priority_df.iloc[-1]['자치구']}")
        
    st.divider()
    
    # [변경됨] 선택된 자치구만 다른 색상으로 강조하여 표시
    # 색상 리스트 만들기 (선택된 구는 주황색, 나머지는 보라색)
    colors = ['#e17055' if gu == selected_gu else '#a29bfe' for gu in priority_df['자치구']]
    
    fig_total = go.Figure(data=[
        go.Bar(
            x=priority_df['자치구'], 
            y=priority_df['CCTV_설치_우선순위_점수'],
            marker_color=colors
        )
    ])
    
    fig_total.update_layout(
        title=f"서울시 전체 CCTV 설치 우선순위 점수 (<b>{selected_gu}</b> 강조)",
...
if prompt := st.chat_input(f"{selected_gu} 치안 데이터에 대해 무엇이든 물어보세요..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("AI가 분석 중입니다..."):
            ...
            # 여기서부터가 누락되어 발생한 에러입니다. 반드시 포함되어야 합니다.
            except Exception as e:
